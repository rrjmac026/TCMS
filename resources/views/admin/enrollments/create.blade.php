...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold dark:text-white" style="color:#003087;">
                <i class="fas fa-plus mr-2" style="color:#CE1126;"></i> Add Enrollment
            </h1>
            <p class="text-sm mt-0.5" style="color:#5a7aaa;">Enroll a trainee in a course</p>
        </div>
    </div>
    ...
    <div class="rounded-2xl border overflow-hidden dark:bg-[#0d1f3c] dark:border-[#1e3a6b]"
        ...
        <form method="POST" action="{{ route('admin.enrollments.store') }}" class="p-8 space-y-6">
            @csrf

            {{-- Trainee --}}
            <div>
                <label class="block text-xs font-700 uppercase tracking-wide mb-1.5" style="color:#5a7aaa;">
                    Trainee <span style="color:#CE1126;">*</span>
                </label>
                <select name="trainee_id"
                        ...
                        style="border-color:{{ $errors->has('trainee_id') ? '#CE1126' : '#c5d8f5' }}; color:#001a4d;"
                        onfocus="this.style.borderColor='#0057B8'; this.style.boxShadow='0 0 0 3px rgba(0,87,184,0.10)'"
                        onblur="this.style.borderColor='{{ $errors->has('trainee_id') ? '#CE1126' : '#c5d8f5' }}'; this.style.boxShadow='none'">
            ...
            <div>
                ...
                <input type="date" name="enrolled_at" value="{{ old('enrolled_at', now()->format('Y-m-d')) }}"
                       class="w-full px-4 py-2.5 rounded-xl border text-sm outline-none transition
                              dark:bg-[#0a1628] dark:border-[#1e3a6b] dark:text-white"
                       style="border-color:{{ $errors->has('enrolled_at') ? '#CE1126' : '#c5d8f5' }}; color:#001a4d;"
                       onfocus="this.style.borderColor='#0057B8'; this.style.boxShadow='0 0 0 3px rgba(0,87,184,0.10)'"
                       onblur="this.style.borderColor='{{ $errors->has('enrolled_at') ? '#CE1126' : '#c5d8f5' }}'; this.style.boxShadow='none'">
                @error('enrolled_at')
                    <p class="text-xs mt-1" style="color:#CE1126;">{{ $message }}</p>
                @enderror
            </div>
