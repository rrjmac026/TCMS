    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold dark:text-white" style="color:#003087;">
                <i class="fas fa-pen mr-2" style="color:#CE1126;"></i> Edit Course
            </h1>
            <p class="text-sm mt-0.5" style="color:#5a7aaa;">Updating details for {{ $course->name }}</p>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('admin.courses.update', $course) }}" class="p-8 space-y-6">
            ...
            <div>
                ...
                <textarea name="description" rows="4"
                          ...
                          onfocus="this.style.borderColor='#0057B8'; this.style.boxShadow='0 0 0 3px rgba(0,87,184,0.10)'"
                          onblur="this.style.borderColor='{{ $errors->has('description') ? '#CE1126' : '#c5d8f5' }}'; this.style.boxShadow='none'">{{ old('description', $course->description) }}</textarea>
                @error('description')
                    <p class="text-xs mt-1" style="color:#CE1126;">{{ $message }}</p>
                @enderror
            </div>
