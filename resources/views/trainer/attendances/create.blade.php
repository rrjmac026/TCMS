...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold dark:text-white" style="color:var(--att-accent);">
                <i class="fas fa-plus mr-2" style="color:var(--att-red);"></i> Record Attendance
            </h1>
            <p class="text-sm mt-0.5" style="color:var(--att-text-sec);">Mark trainee attendance for a session</p>
        </div>
    </div>
    ...
    <div class="rounded-2xl border overflow-hidden dark:bg-[#0d1f3c] dark:border-[#1e3a6b]"
        ...
        <form method="POST" action="{{ route('trainer.attendances.store') }}" class="p-8 space-y-6">
            @csrf
                ...
                <div class="grid grid-cols-3 gap-4">
                    @foreach(['present' => ['label' => 'Present', 'icon' => 'fa-check', 'color' => '#16a34a'], 'late' => ['label' => 'Late', 'icon' => 'fa-clock', 'color' => '#b38a00'], 'absent' => ['label' => 'Absent', 'icon' => 'fa-times', 'color' => '#CE1126']] as $value => $option)
                        <label class="relative cursor-pointer">
                            <input type="radio" name="status" value="{{ $value }}" {{ old('status') === $value ? 'checked' : '' }}
