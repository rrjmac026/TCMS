...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="rounded-2xl border overflow-hidden dark:bg-[#0d1f3c] dark:border-[#1e3a6b]"
        ...
        <form method="POST" action="{{ route('admin.assessments.update', $assessment) }}" class="p-8 space-y-6">
            @csrf
            @method('PUT')
                ...
                <select name="enrollment_id"
                    ...
                    @foreach($enrollments as $enrollment)
                        <option value="{{ $enrollment->id }}"
                            {{ old('enrollment_id', $assessment->enrollment_id) == $enrollment->id ? 'selected' : '' }}>
                ...
                <select name="trainer_id"
                    ...
                    @foreach($trainers as $trainer)
                        <option value="{{ $trainer->id }}"
                            {{ old('trainer_id', $assessment->trainer_id) == $trainer->id ? 'selected' : '' }}>
                            {{ $trainer->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" name="assessed_at"
                       value="{{ old('assessed_at', $assessment->assessed_at?->format('Y-m-d')) }}"
                       class="w-full px-4 py-2.5 rounded-xl border text-sm outline-none transition
                              dark:bg-[#0a1628] dark:border-[#1e3a6b] dark:text-white"
