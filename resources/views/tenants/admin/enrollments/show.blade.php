...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            ...
            <p class="text-sm mt-0.5" style="color:var(--end-text-sec);">{{ $enrollment->trainee->name }} - {{ $enrollment->course->name }}</p>
        </div>
    </div>
    ...
    <div class="rounded-2xl border overflow-hidden dark:bg-[#0d1f3c] dark:border-[#1e3a6b]"
        ...
        <div class="p-8 grid grid-cols-1 sm:grid-cols-2 gap-6">
            @php
                $details = [
                    ['icon' => 'fa-user', 'color' => 'var(--end-accent)', 'bg' => 'var(--end-accent-bg)', 'label' => 'Trainee', 'value' => $enrollment->trainee->name],
    ...
    <div class="rounded-2xl border p-6 dark:bg-[#0d1f3c] dark:border-[#1e3a6b]"
         style="background:var(--end-surface); border-color:rgba(206,17,38,0.25);">
        <h3 class="text-sm font-800 mb-1 flex items-center gap-2" style="color:var(--end-red);">
            <i class="fas fa-triangle-exclamation"></i> Danger Zone
        </h3>
        <p class="text-xs mb-4" style="color:var(--end-text-sec);">Deleting this enrollment is permanent and cannot be undone.</p>
        <form method="POST" action="{{ route('admin.enrollments.destroy', $enrollment) }}"
            ...
            <button type="submit"
                    ...
                    style="background:linear-gradient(135deg,var(--end-red),#A50E1E); box-shadow:0 3px 12px rgba(206,17,38,0.25);">
