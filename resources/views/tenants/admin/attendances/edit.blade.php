    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold dark:text-white" style="color:var(--attf-accent);">
                <i class="fas fa-pen mr-2" style="color:var(--attf-red);"></i> Edit Attendance
            </h1>
            <p class="text-sm mt-0.5" style="color:var(--attf-text-sec);">Updating attendance for {{ $attendance->enrollment->trainee->name }}</p>
        </div>
    </div>
                    ...
                    @foreach (['present' => ['label' => 'Present', 'icon' => 'fa-check-circle', 'color' => 'var(--attf-green)', 'bg' => 'rgba(34,197,94,0.10)'],
                        ...
                        <label class="relative cursor-pointer">
                            ...
                            <div class="p-4 rounded-xl border-2 transition text-center"
                                 style="border-color:{{ old('status', $attendance->status) === $key ? $option['color'] : 'var(--attf-border)' }}; background:{{ old('status', $attendance->status) === $key ? $option['bg'] : 'var(--attf-surface)' }};">
