...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold" style="color:var(--tsf-primary);">
                <i class="fas fa-pen mr-2" style="color:var(--tsf-red);"></i> Edit Training Schedule
            </h1>
            <p class="text-sm mt-0.5" style="color:var(--tsf-muted);">Updating schedule for {{ $trainingSchedule->course->name }}</p>
        </div>
    </div>
    ...
    <div class="rounded-2xl border overflow-hidden"
        ...
        <form method="POST" action="{{ route('admin.training-schedules.update', $trainingSchedule) }}" class="p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="course_id"
                    ...
                    @foreach ($courses as $course)
                        <option value="{{ $course->id }}" {{ old('course_id', $trainingSchedule->course_id) == $course->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select name="trainer_id"
                    ...
                    @foreach ($trainers as $trainer)
                        <option value="{{ $trainer->id }}" {{ old('trainer_id', $trainingSchedule->trainer_id) == $trainer->id ? 'selected' : '' }}>
                            {{ $trainer->name }}
                        </option>
                    @endforeach
                </select>
