...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold dark:text-white" style="color:var(--asf-accent);">
                <i class="fas fa-edit mr-2" style="color:var(--asf-red);"></i> Edit Assessment
            </h1>
            <p class="text-sm mt-0.5" style="color:var(--asf-text-sec);">Update assessment details</p>
        </div>
    </div>
    ...
    <div class="rounded-2xl border overflow-hidden dark:bg-[#0d1f3c] dark:border-[#1e3a6b]"
        ...
        <form method="POST" action="{{ route('trainer.assessments.update', $assessment) }}" class="p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                <label class="block text-xs font-700 uppercase tracking-wide mb-1.5" style="color:var(--asf-text-sec);">
                    Trainee <span style="color:var(--asf-red);">*</span>
                </label>
                <select name="enrollment_id"
                    ...
                    @foreach($enrollments as $enrollment)
                        <option value="{{ $enrollment->id }}" {{ old('enrollment_id', $assessment->enrollment_id) == $enrollment->id ? 'selected' : '' }}>
            ...
            <div>
                <label class="block text-xs font-700 uppercase tracking-wide mb-1.5" style="color:var(--asf-text-sec);">
                    Score (%) <span class="text-xs" style="color:var(--asf-text-sec);">(Optional)</span>
                </label>
                <input type="number" name="score" value="{{ old('score', $assessment->score) }}"
                       placeholder="e.g. 85"
                       min="0"
                       max="100"
                       step="0.5"
            ...
            <div>
                ...
                <select name="result"
                    ...
                    <option value="competent" {{ old('result', $assessment->result) === 'competent' ? 'selected' : '' }}>Competent</option>
                    <option value="not_yet_competent" {{ old('result', $assessment->result) === 'not_yet_competent' ? 'selected' : '' }}>Not Yet Competent</option>
                </select>
                @error('result')
                    <p class="text-xs mt-1" style="color:var(--asf-red);">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <input type="date" name="assessed_at" value="{{ old('assessed_at', $assessment->assessed_at?->format('Y-m-d')) }}"
                       class="w-full px-4 py-2.5 rounded-xl border text-sm outline-none transition
                              dark:bg-[#0a1628] dark:border-[#1e3a6b] dark:text-white"
            ...
            <div>
                <label class="block text-xs font-700 uppercase tracking-wide mb-1.5" style="color:var(--asf-text-sec);">
                    Remarks <span class="text-xs" style="color:var(--asf-text-sec);">(Optional)</span>
                </label>
                <textarea name="remarks" rows="4"
                          ...
                          onfocus="this.style.borderColor='var(--asf-accent)'; this.style.boxShadow='0 0 0 3px rgba(0,87,184,0.10)'"
                          onblur="this.style.borderColor='{{ $errors->has('remarks') ? 'var(--asf-red)' : 'var(--asf-border)' }}'; this.style.boxShadow='none'">{{ old('remarks', $assessment->remarks) }}</textarea>
                @error('remarks')
                    <p class="text-xs mt-1" style="color:var(--asf-red);">{{ $message }}</p>
                @enderror
            </div>
